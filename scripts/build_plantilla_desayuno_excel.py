...
def _write_instrucciones(ws, hotel: Path, n_rec: int, n_ext: int) -> None:
    ...
    lines = [
        ...
        "3. Huespedes: basta en la primera línea de ese día (si vacío, se cuenta 1 por línea Receta).",
        "4. Tipo: Receta (plato/bebida), Extra (buffet rápido) o Producto (mismo catálogo de extras).",
        "5. Nombre: elija del desplegable (lista en hoja Catalogo).",
        "6. Cantidad ↑↓: raciones o unidades. Escriba un número 1–30.",
        "7. Extra1…Extra4: extras sobre una Receta (bacon, aguacate, huevo, pan, etc.).",
        ...
        "• Para huevo suelto (no inglés): Tipo=Receta y Nombre=Huevo frito / pochado / …",
        "  o Tipo=Extra y Nombre=Huevo frito (añade al buffet).",
        "",
        "PAN (tostadas / sándwiches)",
        "• Por defecto las fichas llevan molde común (blanco) = «Tostada» / «Pan blanco».",
        "• Para integral: Extra1 = Tostada integral (o Pan integral).",
        "• Para sin gluten: Extra1 = Tostada sin gluten (o Pan sin gluten)",
        "  → panecillo s/gluten Betina; sustituye el pan de la ficha (1 reb → 1 ud).",
        "• Para volver a blanco: Extra1 = Tostada / Pan blanco.",
        "• Sin pan: Omitir1 = Sin tostada.",
        "• Pan suelto de buffet: Tipo=Extra y Nombre=Tostada / integral / sin gluten.",
        "",
        "9. Guarde el archivo y ejecute el importador (ver 2_importar_a_bm.cmd).",
        "10. Use --dry-run primero. La columna Importado la rellena el script.",
        "",
        "Al regenerar la plantilla NO se borran las filas ya rellenadas en Registro.",
    ]
    for i, line in enumerate(lines, start=2):
        ws[f"A{i}"] = line
        if line.startswith("CÓMO") or line.startswith("HUEVOS") or line.startswith("PAN"):
